...
viewExplanation ({ i18n } as model) =
    ...
    , Accordion.view
        ...
        , content = List.map (Html.map never) <| Translations.interpolationJsonSyntaxBody i18n [ class "highlighted" ]
        , id = "json_syntax"
        }
        model
    , Accordion.view
        { headline = Translations.propertiesHeadline i18n
        , content = List.map (Html.map never) <| Translations.interpolationPropertiesSyntaxBody i18n [ class "highlighted" ]
        , id = "properties_syntax"
        }
        model
    , Accordion.view
        { headline = Translations.fluentHeadline i18n
        , content = List.map (Html.map never) <| Translations.interpolationFluentSyntaxBody i18n [ class "highlighted" ]
        , id = "fluent_syntax"
        }
        model
    , Html.h2 [] [ Html.text <| Translations.interpolationGeneratedCodeHeadline i18n ]
    , Html.map never <| Html.p [] <| Translations.interpolationGeneratedCodeBody i18n [ class "highlighted" ]
