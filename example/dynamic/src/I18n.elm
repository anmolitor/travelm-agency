module I18n exposing (I18n, Language(..), decodeDemo, decodeMessages, demoKey, greeting, init, languageFromString, languageSwitchInfo, languageToString, languages, loadDemo, loadMessages, order, static)
...
will make a `GET` request to /i18n/demo.de.json and will call GotTranslations with the decoded response.
...
loadDemo opts_ =
    Http.get
        ...
        , url = opts_.path ++ "/demo." ++ languageToString opts_.language ++ ".json"
        }
...
will make a `GET` request to /i18n/messages.de.json and will call GotTranslations with the decoded response.
