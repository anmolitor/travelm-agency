...
replacePlaceholders (I18n _ intl_ lang_) argList_ =
    let
        ...
        matchParser_ =
            Parser.succeed Tuple.pair
                ...
                |= (Parser.chompUntil "}"
                        ...
                        |> Parser.andThen
                            (\str_ ->
                                case Json.Decode.decodeString (Json.Decode.dict Json.Decode.string) str_ of
                                    Ok ok_ ->
                                        Parser.succeed ok_

                                    Err err_ ->
                                        Parser.problem (Json.Decode.errorToString err_)
                            )
                   )
                |. Parser.token "}"
        ...
        matchStrings_ n_ ( default_, cases_ ) =
            Dict.get (getArg_ n_) cases_ |> Maybe.withDefault default_
                    ...
                    Parser.oneOf
                        [ Parser.succeed ((++) state_ >> Parser.Loop)
                            ...
                            |= Parser.oneOf
                                ...
                                , Parser.succeed matchStrings_ |. Parser.token "S" |= Parser.int |. Parser.token "|" |= matchParser_
                                ]
    ...
    in
    Debug.log "a" >> Parser.run parser_ >> Debug.log "" >> Result.toMaybe >> Maybe.withDefault ""
...
displayGender ((I18n { demo } _ _) as i18n_) gender_ =
...
matchOnGender ((I18n { demo } _ _) as i18n_) gender_ =
...
matchOnNumbers ((I18n { demo } _ _) as i18n_) amount_ =
    case Array.get 8 demo of
        Just translation_ ->
            replacePlaceholders i18n_ [ String.fromFloat amount_ ] translation_
...
will make a `GET` request to /i18n/demo.de.232734737.json and will call GotTranslations with the decoded response.
...
languageToFileName_demo lang_ =
    case lang_ of
        De ->
            "demo.de.232734737.json"

        En ->
            "demo.en.896249431.json"

        Fr ->
            "demo.fr.4170337794.json"
