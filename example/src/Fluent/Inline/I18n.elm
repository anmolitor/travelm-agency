module Fluent.Inline.I18n exposing (I18n, Language(..), attributes, attributesTitle, attributesWithVarAndTerm, compileTimeDatesAndNumbers, dateTimeFun, de, dynamicTermKey, en, fr, init, languageFromString, languageToString, languages, load, matchOnNumbers, matchOnStrings, nestedTermKey, numberFun, staticTermKey)
...
de =
    ...
    , matchOnNumbers_ =
        \intl_ amount_ ->
            case
                Intl.formatFloat intl_ { number = amount_, language = "de", args = [] }
                    |> String.toFloat
                    |> Maybe.map
                        (\n_ ->
                            Intl.determinePluralRuleFloat intl_ { language = "de", number = n_, type_ = Intl.Cardinal }
                        )
...
en =
    ...
    , matchOnNumbers_ =
        \intl_ amount_ ->
            (case
                Intl.formatFloat intl_ { number = amount_, language = "en", args = [] }
                    |> String.toFloat
                    |> Maybe.map
                        (\n_ ->
                            Intl.determinePluralRuleFloat intl_ { language = "en", number = n_, type_ = Intl.Cardinal }
                        )
                    |> Maybe.withDefault Intl.Other
                    |> Intl.pluralRuleToString
             of
...
fr =
    ...
    , matchOnNumbers_ =
        \intl_ amount_ ->
            (case
                Intl.formatFloat intl_ { number = amount_, language = "fr", args = [] }
                    |> String.toFloat
                    |> Maybe.map
                        (\n_ ->
                            Intl.determinePluralRuleFloat intl_ { language = "fr", number = n_, type_ = Intl.Cardinal }
                        )
                    |> Maybe.withDefault Intl.Other
                    |> Intl.pluralRuleToString
             of
...
matchOnNumbers ( i18n_, intl_ ) =
    i18n_.matchOnNumbers_ intl_


matchOnStrings : I18n -> String -> String
matchOnStrings ( i18n_, intl_ ) =
    i18n_.matchOnStrings_
